...
const API_GET_USER_NOT_RECEIVE_NLLS_INTERNAL_PLAN = import.meta.env.VITE_API_GET_USER_NOT_RECEIVE_NLLS_INTERNAL_PLAN;
const UserAPIService = {
    ...
    getUserNotReceiveNLLSInternalPlan: async (organId, planId) => {
        try {
            const response = await axiosHttpService.get(API_GET_USER_NOT_RECEIVE_NLLS_INTERNAL_PLAN + '/' + organId + '/' + planId);
            return response.data;
        }catch(err){
            console.log(err);
            return null;
