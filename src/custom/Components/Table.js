...
function getStateIdFromDataRow(dataRow) {
  let stateId = "";
  for (let i = 0; i < dataRow.length; i++) {
    if (dataRow[i].props === undefined) continue;
    if (dataRow[i].props.stateId != undefined) return dataRow[i].props.stateId;
  }
  return stateId;
}
      ...
      if (!Array.isArray(row[key])) {
        if (key.includes("state")) {
          ...
          data.push(
            <td
              stateId={`${stateId}`}
              className="text-white px-[12px] py-[16px] break-words font-medium text-center"
            >
              <span
                className={`block px-[2px] py-[4px] rounded-md text-[12px] font-300 break-normal ${color}`}
              >
                {row[key]}
              </span>
            </td>
          ...
          data.push(
            <td className="px-[12px] py-[16px] break-words text-center">
              {row[key]}
            </td>
          );
        }
        ...
        data.push(
          <td className="px-[12px] py-[16px] overflow-hidden">
...
export const Table = ({
  ...
  const dataTableRenderForTableCell =
    ConvertDataToArrayForRenderTableCell(fieldDatas);
  ...
  const handleCheckBoxAll = (e) => {
    ...
    setIsCheck(fieldDatas.map((file) => "checkbox" + "-" + file["id"] + "-" + file["state"]));
    if (isCheckAll) {
      setIsCheck([]);
    }
  };
        ...
        <div className="bg-[#f0f2f5] rounded-[2px] border-[2px] border-solid border-stone-400">
          <table className="table-fixed w-full">
            ...
            <thead className="bg-[#fafafa]">
              <tr>
                {doesNeedOrder && (
                  <th className="text-[12px]  relative w-[40px] text-center px-[8px] py-[12px] before:content-[''] before:w-[2px] before:absolute before:right-0 before:h-[20px] before:bg-[#e0e0e0] before:top-[50%] before:translate-y-[-50%]">
                    TT
                  </th>
                )}
                ...
                {fieldNames.map((field, index) => {
                  let className =
                    "text-[12px] relative text-center px-[8px] py-[12px]";
                  if (index < fieldNames.length - 1) {
                    className +=
                      " before:content-[''] before:w-[2px] before:absolute before:right-0 before:h-[20px] before:bg-[#e0e0e0] before:top-[50%] before:translate-y-[-50%]";
                  }
                  return (
                    <th
                      key={GetKey()}
            ...
            <tbody>
                  ...
                  <td colSpan={13}>
                    <div className="w-full bg-white text-gray-400">
                      <div className="text-center p-[16px]">
                        <div>
                          <i className="text-[50px] fa-solid fa-box-open"></i>
                        </div>
              ...
              {dataTableRenderForTableCell.map((dataRow, index) => {
                return (
                  <tr
                    className="hover:bg-[#ecebeb] bg-white border-t-[1px] border-solid border-[#e0e0e0] text-[13px]"
                    key={GetKey()}
                  >
                    {doesNeedOrder && (
                      <td className="text-center px-[12px] py-[16px]">
                        <span className="block w-[24px] h-[24px] rounded-[50%] bg-[#ccc]">
                          {index + 1}
                        </span>
                      </td>
                    )}
                    ...
                    {isCheckBox && (
                      <td className="px-[12px] py-[16px] overflow-hidden text-center">
                        <CheckBox
                          key={GetKey()}
                          type="checkbox"
                          id={"checkbox" + "-" + dataRow[0] + "-" + getStateIdFromDataRow(dataRow)}
                          handleClickCheckBox={handleClickCheckBox}
                          isChecked={isCheck.includes("checkbox" + "-" + dataRow[0] + "-" + getStateIdFromDataRow(dataRow))}
                        />
                      </td>
                    )}
                    {
