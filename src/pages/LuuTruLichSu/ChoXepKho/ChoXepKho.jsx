...
import { XEP_KHO_KE_HOACH_NOP_LUU_LICH_SU } from "src/storage/StorageOffice";
...
const ChoXepKho = () => {
    ...
    const reFetchData = async () => {
        setIsLoading(true);
        const plansRaw = await PlanAPIService.getChoXepKhoLichSuPlan();
        const plans = [];
    ...
    return <div className="w-full">
        <div className="w-full px-[24px] pt-[12px] pb-[16px] bg-white">
            <p className="text-[14px] font-300 cursor-pointer ">
                <span className="text-[rgba(0,0,0,.45)]">
                    <Link to="/luu-tru-lich-su/cho-xep-kho">
                        Lưu trữ lịch sử /{" "}
                    </Link>
                </span>
                <span>
        ...
        <div className="w-full px-[24px] pb-[16px] bg-white flex justify-between">
            <p className="text-[20px] font-bold ">Chờ xếp kho</p>
        </div>
        ...
        <Table
            ...
            fieldNames={XEP_KHO_KE_HOACH_NOP_LUU_LICH_SU}
            fieldDatas={plan}
            isLoading={isLoading}
            isCheckBox={true}
