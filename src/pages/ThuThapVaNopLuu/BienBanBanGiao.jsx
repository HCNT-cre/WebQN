...
const Create = ({
    ...
    const handleOk = async () => {
        await axiosHttpService.post(`${API_COLLECT_SUBMIT_DELIVERY_RECORDS}`, request);
        setTimeout(() => {
            reFetchData();
            setRequest({
                state: "Tạo mới"
...
const Delete = ({ id, reFetchData }) => {
    ...
    const handleConfirm = async () => {
        const deletePlan = async () => {
            await axiosHttpService.delete(API_COLLECT_SUBMIT_DELIVERY_RECORDS + id);
        };
...
const Update = ({ reFetchData, id }) => {
    ...
    const getPlan = async () => {
        const { data } = await axiosHttpService.get(API_COLLECT_SUBMIT_DELIVERY_RECORDS + id);
        setRequest({
            name: data.name,
            date: data.date,
            organ: data.organ,
    ...
    const handleOk = async () => {
        await axiosHttpService.put(API_COLLECT_SUBMIT_DELIVERY_RECORDS + id, request);
        setModalOpen(false);
        reFetchData();
    };
...
const BienBanBanGiao = () => {
    ...
    const reFetchData = useCallback(async () => {
        setIsLoading(true);
        const res = await axiosHttpService.get(`${API_COLLECT_SUBMIT_DELIVERY_RECORDS}`);
