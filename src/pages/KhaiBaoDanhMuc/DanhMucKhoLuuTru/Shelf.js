...
const API_STORAGE_DELETE_SHELF = process.env.REACT_APP_API_STORAGE_DELETE_SHELF
const API_STORAGE_PUT_SHELF = process.env.REACT_APP_API_STORAGE_PUT_SHELF
...
const Update = ({ reFetchData, id }) => {
    ...
    const handleOk = async () => {
        if (request.name !== null)
            await axios.put(API_STORAGE_PUT_SHELF + id, request)
        setModalOpen(false)
        reFetchData()
    }
...
const Delete = ({ id, reFetchData }) => {
    ...
    const handleConfirm = () => {
        const deleteWarehouse = async () => {
            const res = await axios.delete(API_STORAGE_DELETE_SHELF + id)
            console.log(res)
        }
