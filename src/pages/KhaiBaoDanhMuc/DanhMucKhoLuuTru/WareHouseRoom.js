...
const API_STORAGE_DELETE_WAREHOUSEROOM = process.env.REACT_APP_API_STORAGE_DELETE_WAREHOUSEROOM
const API_STORAGE_PUT_WAREHOUSEROOM = process.env.REACT_APP_API_STORAGE_PUT_WAREHOUSEROOM
const API_STORAGE_GET_WAREHOUSE_ALL = process.env.REACT_APP_API_STORAGE_GET_WAREHOUSE_ALL

const Search = Input.Search


const Create = ({ modalOpen, setModalOpen, optionOrgan, reFetchData, optionWarehouse }) => {
    ...
    const handleOk = async () => {
        await axios.post(API_STORAGE_GET_WAREHOUSEROOM_ALL, request)
        setModalOpen(false)
        reFetchData()
...
const SearchBar = ({ optionOrgan, optionWarehouse }) => {
    return (
...
const Update = ({ reFetchData, id }) => {
    ...
    const handleOk = async () => {
        if (request.name !== null)
            await axios.put(API_STORAGE_PUT_WAREHOUSEROOM + id, request)
        setModalOpen(false)
        reFetchData()
    }
...
const Delete = ({ id, reFetchData }) => {
    ...
    const handleConfirm = () => {
        const deleteWarehouse = async () => {
            const res = await axios.delete(API_STORAGE_DELETE_WAREHOUSEROOM + id)
            console.log(res)
        }
...
const WareHouseRoom = () => {
    ...
    return (
        <DanhMucKhoLuuTru title="Phòng kho" fieldNames={WARE_HOUSE_ROOM} fieldDatas={wareHouseRoom} isLoading={isLoading} SearchBar={<SearchBar optionOrgan={optionOrgan} optionWarehouse={optionWarehouse} />} Create={<Create optionOrgan={optionOrgan} reFetchData={reFetchData} optionWarehouse={optionWarehouse} />} />
    )
}
