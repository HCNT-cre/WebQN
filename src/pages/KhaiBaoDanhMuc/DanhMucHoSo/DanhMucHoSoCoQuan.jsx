    ...
    return (
        ...
        >
            <div>
                ...
                <div className="flex justify-between py-[12px]">
                    <span>Năm</span>
                    <Input name="year" type="text" className="w-[70%]" value={year} disabled />
                </div>
...
const DanhMucHoSoCoQuan = () => {
    const [isLoading, setIsLoading] = useState(true)
    const [order, setOrder] = useState(1)
    const [parent, setParent] = useState(null)
    const [modalOpen, setModelOpen] = useState(false)
    const [modalChildOpen, setModelChildOpen] = useState(false)
    ...
    const reFetchData = async () => {
        setIsLoading(true)
        const data = await CategoryAPIService.getCategoryFileByOrganAndYear(params.organ_id, params.year)
        setData(data)
        setIsLoading(false)
    }

    const columnsFirst = generateColumn(reFetchData, setModelChildOpen, setOrder, setParent, 2)
    const columnsSecond = generateColumn(reFetchData, setModelChildOpen, setOrder, setParent, 3)
    const columnsThird = generateColumn(reFetchData, setModelChildOpen, setOrder, setParent)
    ...
    useEffect(() => {
        const fetchOrgan = async () => {
            const organ = await OrganAPIService.getOrganById(params.organ_id)
            setOrgan(organ)
        }
        fetchOrgan()
    }, [params.organ_id])
        ...
        <div className="w-full">
            <div className="w-full px-[24px] pt-[12px] pb-[16px] bg-white">
                <p className="text-[14px] font-300 cursor-pointer ">
                    <span className="text-[rgba(0,0,0,.45)]">Khai báo danh mục / </span> 
                    <Link className="text-[rgba(0,0,0,.45)]" to="/khai-bao-danh-muc/danh-muc-ho-so"> Danh mục hồ sơ /</Link> 
                    <Link className="text-[rgba(0,0,0,.45)]" to={`/khai-bao-danh-muc/danh-muc-ho-so/${params.year}`}> Năm /</Link> 
                    <Link className="text-[rgba(0,0,0,.45)]" to={`/khai-bao-danh-muc/danh-muc-ho-so/${params.year}/${params.organ_id}`}> Cơ quan </Link>
                </p>

            </div>
            <div className="w-full px-[24px] pb-[16px] bg-white flex justify-between">
                <p className="text-[20px] font-bold">Danh mục hồ sơ / Năm / Cơ quan</p>
                ...
                <CreateOfCategoryFile
                    modalOpen={modalChildOpen}
                    setModelOpen={setModelChildOpen}
                    reFetchData={reFetchData}
                    order={order}
                    select={data}
                    parent={parent}
...
export default DanhMucHoSoCoQuan
