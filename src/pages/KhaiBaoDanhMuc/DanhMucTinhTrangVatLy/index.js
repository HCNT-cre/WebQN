...
import { PHYSICAL_STATE, PHYSICAL_STATE_INPUT } from "storage/Storage"
...
const API_PHYSICAL_STATE = process.env.REACT_APP_API_PHYSICAL_STATE
...
const Form = ({ modalOpen, setModalOpen, id, fetchFieldData }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!id) return
            try {
                const res = await axios.get(API_PHYSICAL_STATE + id)
                const data = res.data
                setRequest(data)
            } catch (err) {
                console.log(err);
    ...
    const onSubmit = async (e) => {
        ...
        if (id !== null) {
            await axios.put(API_PHYSICAL_STATE + id, request)
            notifySuccess("Cập nhật tình trạng vật lý thành công")
        } else {
            await axios.post(API_PHYSICAL_STATE, request)
...
const KhaiBaoDanhMucTinhTrangVatLy = () => {
    ...
    const fetchFieldData = async () => {
        setIsLoading(true)
        const res = await axios.get(API_PHYSICAL_STATE)
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(API_PHYSICAL_STATE + id)
